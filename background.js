...
async function handleGeminiAnalysis(sendResponse) {
  try {
    ...
    // 4. Call Gemini
    const prompt = `Tell me the total duration of this video and write a very short summary from this transcript: ${transcriptResponse.data}`;
    ...
    if (result.candidates && result.candidates[0].content.parts[0].text) {
      sendResponse({ success: true, data: result.candidates[0].content.parts[0].text });
    } else {
